<script lang="ts">
	...
	let testimonios = $state<Testimonio[]>([]);
	let cargandoTestimonios = $state(true);
	let nombreTestimonio = $state("");
	let textoTestimonio = $state("");
	let estrellasTestimonio = $state(5);
	let guardandoTestimonio = $state(false);

	const detenerTestimonios = escucharTestimonios(
		(datos) => {
			testimonios = datos;
			cargandoTestimonios = false;
		},
		(err) => {
			error = `No se pudieron cargar los testimonios: ${err.message}`;
			cargandoTestimonios = false;
		},
	);

	async function crearTestimonio(evento: SubmitEvent) {
		evento.preventDefault();
		guardandoTestimonio = true;
		error = "";

		try {
			await agregarTestimonio({
				nombre: nombreTestimonio,
				texto: textoTestimonio,
				estrellas: estrellasTestimonio,
				orden:
					testimonios.reduce(
						(maximo, t) => Math.max(maximo, t.orden),
						-1,
					) + 1,
			});
			...
			avisar("Testimonio agregado.");
		} catch (err) {
			error =
				err instanceof Error
					? err.message
					: "No se pudo agregar el testimonio.";
		} finally {
			guardandoTestimonio = false;
		}
	}

	async function moverTestimonio(
		testimonio: Testimonio,
		direccion: -1 | 1,
	) {
		const indice = testimonios.findIndex((t) => t.id === testimonio.id);
		const vecino = testimonios[indice + direccion];
		if (!vecino) return;

		try {
			await Promise.all([
				actualizarTestimonio(testimonio.id, { orden: vecino.orden }),
				actualizarTestimonio(vecino.id, { orden: testimonio.orden }),
			]);
		} catch (err) {
			error =
				err instanceof Error ? err.message : "No se pudo reordenar.";
	...
	async function borrarTestimonio(testimonio: Testimonio) {
		if (!confirm(`¿Eliminar el testimonio de ${testimonio.nombre}?`)) return;

		try {
			await eliminarTestimonio(testimonio.id);
...
<section class="mx-auto max-w-5xl px-4 py-12 sm:px-6">
	...
	<!-- ------------------------ TESTIMONIOS ------------------------ -->
	<h2 class="mt-14 font-Manrope text-2xl text-slate-700">Testimonios</h2>

	<p class="mt-1 text-sm text-slate-500">
		Aparecen en la portada, entre Instagram y "Nuestra esencia". Copia
		aquí las mejores reseñas que te dejen en Google o por WhatsApp.
	</p>

	<form onsubmit={crearTestimonio} class="mt-5 rounded-2xl bg-slate-50 p-5">
		<div class="grid gap-4 sm:grid-cols-[1fr_auto]">
			<div class="flex flex-col gap-1">
				<label
					for="nombreTestimonio"
					class="text-sm font-semibold text-slate-600"
				>
			...
			<div class="flex flex-col gap-1">
				<label
					for="estrellasTestimonio"
					class="text-sm font-semibold text-slate-600"
				>
					Estrellas
				</label>
				<select
					id="estrellasTestimonio"
					bind:value={estrellasTestimonio}
					class="rounded-lg border border-slate-200 bg-white px-3 py-2"
				>
					{#each [5, 4, 3, 2, 1] as valor}
						<option value={valor}>
		...
		<div class="mt-4 flex flex-col gap-1">
			...
			<textarea
				id="textoTestimonio"
				rows="3"
				bind:value={textoTestimonio}
				required
				placeholder="Los productos llegaron rapidísimo y mi piel cambió por completo..."
				class="rounded-lg border border-slate-200 px-3 py-2"
			></textarea>
		</div>
		...
		<button
			...
			class="mt-5 h-11 rounded-full bg-slate-700 px-6 font-semibold text-white transition hover:bg-slate-600 disabled:opacity-60"
		>
			{guardandoTestimonio ? "Guardando..." : "Agregar testimonio"}
		</button>
	</form>

	<div class="mt-6">
		{#if cargandoTestimonios}
			<p class="text-slate-500">Cargando testimonios...</p>
		{:else if testimonios.length === 0}
			<div
				class="rounded-2xl border border-dashed border-slate-300 p-8 text-center"
			>
				<p class="text-slate-500">
					No hay testimonios. La sección no se mostrará en la portada
					(salvo el botón para dejar reseña).
				</p>
			</div>
		{:else}
			<div class="grid gap-3 md:grid-cols-2">
				{#each testimonios as testimonio, indice (testimonio.id)}
					<article
						class="rounded-2xl border border-slate-200 bg-white p-4"
					>
						<p class="text-amber-500">
							{"★".repeat(testimonio.estrellas)}<span
								class="text-slate-300"
								>{"★".repeat(5 - testimonio.estrellas)}</span
							>
						</p>
						...
						<div class="mt-3 flex items-center gap-1">
							<button
								...
								aria-label="Mover antes"
								disabled={indice === 0}
								onclick={() => moverTestimonio(testimonio, -1)}
								class="rounded-full p-1.5 text-slate-500 transition hover:bg-slate-100 disabled:opacity-30"
							>
								<Icon
							...
							<button
								type="button"
								aria-label="Mover después"
								disabled={indice === testimonios.length - 1}
								onclick={() => moverTestimonio(testimonio, 1)}
								class="rounded-full p-1.5 text-slate-500 transition hover:bg-slate-100 disabled:opacity-30"
							>
								<Icon
							...
							<button
								type="button"
								onclick={() => borrarTestimonio(testimonio)}
								class="ml-auto rounded-full bg-red-50 px-3 py-1 text-xs font-semibold text-red-600 hover:bg-red-100"
							>
								Eliminar
							</button>
